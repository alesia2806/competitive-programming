...
**Greedy simulation with a min-heap:** Process bombs in order of decreasing available time. At each step `t`, the bomb being processed must satisfy `T[i][j] - t >= X`. A bomb becomes available once both its children have been processed. Track the minimum `T[i][j] - t` across all processed bombs. That minimum is the answer X.
